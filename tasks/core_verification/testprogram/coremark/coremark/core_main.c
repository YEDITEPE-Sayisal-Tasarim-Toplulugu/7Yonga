...
#error "Please define a way to initialize a memory block."
    ...
    if (results[0].iterations == 0)
    {
        ...
        while (secs_passed < (secs_ret)1)
        {
            ...
            iterate(&results[0]);
            stop_time();
            secs_passed = time_in_secs(get_time_in_bare());
        }
    ...
    stop_time();
    // ee_printf("iteration bitti.\n\r");

    total_time = get_time_in_bare();
...
//#if HAS_FLOAT
        if (known_id == 3)
        {
            ee_printf("CoreMark 1.0 : 0x%08x / %s %s",
                      default_num_contexts * results[0].iterations
                          / time_in_secs(total_time),
                      COMPILER_VERSION,
                      COMPILER_FLAGS);
